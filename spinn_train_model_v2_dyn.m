...
% 关键稳态修复（见标注 %% [FIX]）：
%   - 仅按前 25 列过滤样本（避免误删 t_hit=NaN 行）   [FIX]
%   - ω 的 NaN/过小值统一兜底                          [FIX]
%   - ddq 求解前有限性检查 & 条件数自适应正则           [FIX]
%   - F10.condM 防 NaN；仅收集“干净样本”               [FIX]
%   - OU 噪声裁剪，防极端阻尼                          [FIX]
%   - 采样后 X_all 为空时立刻友好报错                   [FIX]
    ...
    % 可选：近目标自动收功率门控（默认空，不启用）
    if ~isfield(opts,'Pmax_gate'), def.Pmax_gate = []; end
    ...
    PM = PM(:,1:29);
    % 只按 p25 检查有限性；w/t_hit 可为 NaN，不影响滚动采样  [FIX]
    ...
    X_all = [];   % N x 35
    Y_all = [];   % N x 3  (w)
    L = [0.24 0.214 0.324]; g = 9.81;      % 固定几何（与推理核一致）
    fprintf('收集动态样本...\n');
    ...
    for i = 1:size(PM,1)
        ...
        for k = 1:numel(tvec)
            % —— 4.1 动态特征（按推理口径）——
            F10 = build_features_v2_safe(q, dq, qT, m, L, g, dlnetH, muHv, sigHv, opts.useGPU);  % [FIX]
            X = [p25, F10];    % 25 + 10 = 35

            % —— 4.2 教师权重 w_teacher(t) —— 
            w_teacher = teacher_w_safe(opts.teacher, q, dq, qT, p25, Pmax, Prated, ...
                                       L, opts, T, opts.w_floor);                             % [FIX]
            ...
            omega   = dq;
            omega_nz = omega;
            bad = ~isfinite(omega_nz) | (abs(omega_nz) < opts.omega_eps);
            sgn = sign(tau_des); sgn(sgn==0) = 1;
            omega_nz(bad) = opts.omega_eps .* sgn(bad);
            ...
            s_tot = sum(abs(p_des));
            Pmax_eff = Pmax;
            if isa(opts.Pmax_gate, 'function_handle') %#ok<ISFLG>
